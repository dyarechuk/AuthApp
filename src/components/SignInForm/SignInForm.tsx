	...
	const onSubmitHandler = (data: FormValues) => {
		mutate(data, {
			onSuccess: () => {
				...
				toastSuccess('Successful sign in');
			},
			onError: (error) => {
	...
	return (
		<div className="flex flex-col gap-8 items-center bg-slate-700 rounded-2xl w-full md:w-[40vw] mx-auto pb-8">
			...
			<FormProvider {...methods}>
				...
				<span className="text-lg font-normal text-slate-200 flex flex-col lg:inline-block">
					Don’t have an account?{' '}
					<Link href="/sign-up" className="font-bold hover:text-slate-100 transition">
						Create a Veel Account
					</Link>
				</span>
			</FormProvider>
		</div>
